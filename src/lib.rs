...
pub fn evaluate(expression: &str, context: &HashMapContext) -> Value {
    ...
    let formula = parser::parse_formula(expression, None::<NoCustomFunction>);
    let result = calculate::calculate_formula(formula, Some(&ref_function));
    result
}

pub fn parse_references_string(s: &str) -> Vec<String> {
    let formula = parser::parse_formula(s, None::<NoCustomFunction>);
    formula.get_references()
}

pub fn parse_deps(s: &str) -> Vec<Dependents> {
    let formula = parser::parse_formula(s, None::<NoCustomFunction>);
    formula.get_deps()
}
...
mod test {
    ...
    fn test_simple_formula() {
        let formula = parser::parse_formula(&"=1+1", None::<NoCustomFunction>);
    ...
    fn test_complex_formula() {
        let formula = parser::parse_formula(&"=(1+1)*2", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
    ...
    fn test_date_functions() {
        ...
        let formula = parser::parse_formula(&"=DAYS(end, start)", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, Some(&data_function));
        assert_eq!(result.str_from().as_str(), "182");

        let formula = parser::parse_formula(&"=start+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, Some(&data_function));
        assert!(result.str_from().as_str().contains("2019-03-02"));

        let formula = parser::parse_formula(&"=end-3", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, Some(&data_function));
    ...
    fn test_custom_function() {
        ...
        let formula = parser::parse_formula(&"=Increase(1)+1", Some(&custom_functions));
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "3");

        let formula = parser::parse_formula(&"=EqualFive()+1", Some(&custom_functions));
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "6");

        let formula = parser::parse_formula(&"=SimpleSum(1,2)", Some(&custom_functions));
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "3");
    }
    #[test]
    fn test_formula() {
        ...
        let formula = parser::parse_formula(&"=1+1+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "3");

        let formula = parser::parse_formula(&"=1+1+1+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "4");

        let formula = parser::parse_formula(&"=1+1+1+1+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "5");

        let formula = parser::parse_formula(&"=1+1+1+1+1+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "6");

        let formula = parser::parse_formula(&"=1+1+1+1+1+1+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "7");

        let formula = parser::parse_formula(&"=1+1+1+1+1+1+1+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
        assert_eq!(result.str_from().as_str(), "8");

        let formula = parser::parse_formula(&"=1+1+1+1+1+1+1+1+1", None::<NoCustomFunction>);
        let result = calculate::calculate_formula(formula, None::<NoReference>);
    ...
    fn test_evaluate_with_cell_context() {
        let mut context = HashMapContext::new();
        let _ = context.set_value("A1".to_string(), Value::Number(1.0));
        let _ = context.set_value("A2".to_string(), Value::Number(2.0));
    ...
    fn test_evaluate_with_sheet_cell_context() {
        let mut context = HashMapContext::new();
        let _ = context.set_value("Sheet1!A1".to_string(), Value::Number(1.0));
        let _ = context.set_value("Sheet1!A2".to_string(), Value::Number(2.0));
