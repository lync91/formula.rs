...
fn evaluate_formula_number(s: &str) -> f64 {
    let formula = xfeval::parse_formula(s, None::<NoCustomFunction>);
...
fn evaluate_formula_number_with_custom_function(
    ...
    let formula = xfeval::parse_formula(s, custom_function);
    let result = calculate::calculate_formula(formula, None::<NoReference>);
...
fn it_evaluate_references() {
    let data_function = |s: &str| match s {
        ...
        "B" => Value::Number(2.0),
        "C" => Value::Number(3.0),
        "fix_rate" => Value::Number(10.0),
        "input." => Value::Number(2.0),
        "D" => Value::Number(1.0),
        "F" => Value::Text("=D+1".to_string()),
        "G" => Value::Text("=F+1+D+1".to_string()),
        "Test" => {
            let mut vec = Vec::new();
            vec.push(Value::Number(100.0));
            vec.push(Value::Number(200.0));
            vec.push(Value::Number(300.0));
            Value::Iterator(vec)
        }
...
fn it_evaluate_references_boolean_formulas() {
    let data_function = |s: &str| match s {
        "A" => Value::Boolean(Boolean::True),
        "B" => Value::Boolean(Boolean::False),
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
...
fn it_evaluate_references_error_value_formulas() {
    let data_function = |s: &str| match s {
        "A" => Value::Boolean(Boolean::True),
        "B" => Value::Error(ValueError::Value), //Value::Boolean(Boolean::False),
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
        evaluate_formula_boolean_with_reference(&"=AND(A,B)", Some(&data_function)),
...
fn it_evaluate_references_with_dot() {
    let data_function = |s: &str| match s {
        "A.B" => Value::Number(1.0),
        "B.C" => Value::Number(2.0),
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
...
fn it_evaluate_custom_functions_() {
    let custom_functions = |s: &str, params: Vec<f64>| match s {
        ...
        "CustomSum" => Value::Number(params[0] + params[1] + params[2]),
        "EqualFive" => Value::Number(5.0),
        "CountText" => Value::Text(10.0.to_string()),
...
fn it_evaluates_blanks_in_boolean_operations() {
    let data_function = |s: &str| match s {
        "T" => Value::Boolean(Boolean::True),
        "B" => Value::Blank,
        "F" => Value::Boolean(Boolean::False),
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
...
fn it_evaluates_blank_constructors() {
    let custom_functions = |s: &str, _params: Vec<f64>| match s {
        "BLANK" => Value::Blank,
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
...
fn it_evaluates_blank_in_iterators() {
    let data_function = |s: &str| match s {
        "A" => Value::Number(100.0),
        "Array" => Value::Iterator(vec![Value::Number(100.0), Value::Blank, Value::Blank]),
        "B" => Value::Blank,
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
...
fn it_evaluates_if_formulas() -> Result<(), ParseError> {
    ...
    let data_function = |s: &str| match s {
        "date1" => Value::Date(date1),
        "date2" => Value::Date(date2),
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
...
fn it_evaluates_if_formulas_with_text() {
    let data_function = |s: &str| match s {
        "ReferenceKey" => Value::Text("100".to_string()),
        "ReferenceName" => Value::Text("Test".to_string()),
        _ => Value::Error(ValueError::Value),
    };
    assert_eq!(
